# -*- coding: utf-8 -*-
"""515880.SH 通信ETF 2025 日线：PenZoneA1EtfV1 有/无 A1 中枢过滤对比（T+1 只做多，年末强平）"""
...
SCENARIOS = [
    {
        "key": "with_a1",
        "label": "PenZoneA1EtfV1+A1中枢",
        "root_tag": "_bt_pen_zone_a1_stop_etf_zhiyue_t1_20250101_20260101_daily",
        "run_if_missing": False,
    },
    {
        "key": "no_a1",
        "label": "PenZoneA1EtfV1无A1中枢",
        "root_tag": "_bt_pen_zone_a1_no_a1_etf_t1_515880SH_20250101_20260101_daily",
        "run_if_missing": True,
    },
]

OUTPUT_CSV = os.path.join(
    _EXAMPLES_DIR, "_portfolio_nav_results", "515880_a1_zone_compare_2025.csv"
)
...
class PenZoneA1EtfStrategyDailyNoA1Zone(PenZoneA1EtfStrategy):
    """T+1 只做多，关闭 A1 中枢过滤（有 2 个 A0 即用 A0_01 作中枢参考）。"""
...
def _run_no_a1_backtest(bt_root: str) -> None:
    ...
    dummy = DummyBacktest(
        strategy=PenZoneA1EtfStrategyDailyNoA1Zone,
    ...
    print(f"回测: {SYMBOL} | PenZoneA1EtfV1 无A1中枢 | {SDT}~{EDT} | {FREQ}")
...
def _scenario_metrics(bt_root: str, label: str) -> Dict:
    ...
    return {
        ...
        "频率": FREQ,
        "T+1": "是",
        "A1中枢": "是" if "无" not in label else "否",
        "ATR止损": "是",
        "交易次数": trades,
