# -*- coding: utf-8 -*-
"""PenZoneA1EtfStrategy 指月自选 ETF 30分钟回测 2025（T+1 只做多，ATR 止损）

说明：
- 优先使用 `czsc_research_cache` 中的 1分钟 parquet（可完整覆盖历史），再 resample 到 30分钟
- 若投研缓存仅有日线/无分钟数据，则回退到 akshare(Sina) 30m；该源通常有“滚动窗口限制”，
  2025 可能只能覆盖后半年，脚本会记录每个标的可用的 30m 数据范围

输出目录：
examples/_bt_pen_zone_a1_stop_etf_zhiyue_t1_20250101_20260101_30m
"""
...
ROOT_TAG = f"_bt_pen_zone_a1_stop_etf_zhiyue_t1_{SDT}_{EDT}_30m"
...
def _research_has_minute(symbol: str) -> bool:
    ...
    # robust check: minute data should have much smaller median spacing than daily data
    diffs = dt_ser.diff().dropna()
    if diffs.empty:
        return False
    med_seconds = diffs.median().total_seconds()
    # <= 6 hours roughly indicates intraday bars exist (1m/5m/15m/30m/60m etc.)
...
def probe_symbol_30m(symbol: str) -> Tuple[str, pd.Timestamp | None, pd.Timestamp | None, int]:
    """返回 (source, dt_min, dt_max, n_bars)"""
    if _research_has_minute(symbol):
        try:
            bars = get_raw_bars_research(symbol, FREQ, SDT, EDT, fq="前复权")
            if bars:
                dts = pd.to_datetime([b.dt for b in bars])
                return "research_cache", dts.min(), dts.max(), int(len(bars))
            return "research_cache", None, None, 0
        except Exception:
            # some cached files might be non-minute / malformed; treat as unavailable
            return "research_cache_invalid", None, None, 0

    return "no_minute_in_research_cache", None, None, 0


def get_raw_bars_zhiyue_30m(symbol, freq, sdt, edt, fq="前复权", use_akshare: bool = False, **kwargs):
    """30m 数据读取：research_cache(1m->30m) 优先；可选回退到 akshare 30m"""
...
def get_raw_bars_zhiyue_30m_entry(symbol, freq, sdt, edt, fq="前复权", **kwargs):
    """DummyBacktest 多进程需要可 pickle 的顶层函数入口。"""
    return get_raw_bars_zhiyue_30m(symbol, freq, sdt, edt, fq=fq, use_akshare=USE_AKSHARE, **kwargs)
...
def export_trades_with_names(bt_root: str, output_csv: str, name_map: Dict[str, str]) -> pd.DataFrame:
    df = export_trades_detail(
        bt_root=bt_root,
        pos_name=POS_NAME,
        scenario="etf_zhiyue_t1_2025_30m",
        year_filter=True,
    )
    if df.empty:
...
def main():
    parser = argparse.ArgumentParser(description="指月ETF 30m 回测（2025，T+1 long-only）")
    parser.add_argument(
        "--use-akshare",
        action="store_true",
        help="若 research_cache 无分钟数据，则回退使用 akshare(Sina) 30m（可能只覆盖 2025 后半年，且速度较慢）",
    )
    parser.add_argument("--n-jobs", type=int, default=N_JOBS)
    args = parser.parse_args()
    global USE_AKSHARE
    USE_AKSHARE = bool(args.use_akshare)
    ...
    for i, sym in enumerate(symbols, start=1):
        source, dt_min, dt_max, n_bars = probe_symbol_30m(sym)
        if args.use_akshare and source != "research_cache":
            try:
                df = _fetch_30m_akshare(sym, SDT, EDT)
                if not df.empty:
                    source = "akshare_sina_30m"
                    dt_min, dt_max, n_bars = df["dt"].min(), df["dt"].max(), int(len(df))
            except Exception:
                pass
        probe_rows.append(
            {
                ...
                "覆盖完整2025": int(dt_min is not None and dt_min <= YEAR_START and dt_max is not None and dt_max >= YEAR_END),
            }
        )
        if i % 10 == 0:
            print(f"[probe] {i}/{len(symbols)} done ...")
    probe_df = pd.DataFrame(probe_rows).sort_values(["覆盖完整2025", "2025可用K线数"], ascending=[False, False])
    probe_csv = os.path.join(summary_dir, "data_availability_30m_2025.csv")
    probe_df.to_csv(probe_csv, index=False, encoding="utf-8-sig")

    if args.use_akshare:
        run_symbols = symbols
    else:
        run_symbols = probe_df.loc[probe_df["数据源"] == "research_cache", "代码"].astype(str).tolist()

    dummy = DummyBacktest(
        strategy=PenZoneA1EtfStrategy,
        read_bars=get_raw_bars_zhiyue_30m_entry,
    ...
    print(
        f"执行回测标的数: {len(run_symbols)} / {len(symbols)} | "
        f"{'包含 akshare 回退' if args.use_akshare else '仅 research_cache 有分钟数据的标的'}"
    )
    dummy.execute(run_symbols, n_jobs=int(args.n_jobs))

    # --- 汇总 ---
    sym_df = per_symbol_summary(bt_root, name_map, symbols)
    sym_csv = os.path.join(summary_dir, "etf_zhiyue_t1_per_symbol_2025_30m.csv")
    sym_df.to_csv(sym_csv, index=False, encoding="utf-8-sig")

    trades_csv = os.path.join(summary_dir, "etf_zhiyue_t1_trades_detail_2025_30m.csv")
    ...
    nav_res = analyze_backtest(bt_root, cfg, scenario_name="etf_zhiyue_t1_2025_30m")
    ...
    metrics = {
        ...
        "覆盖完整2025标的数": int(probe_df["覆盖完整2025"].sum()),
    }
    metrics_csv = os.path.join(summary_dir, "etf_zhiyue_t1_metrics_2025_30m.csv")
