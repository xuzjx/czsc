# -*- coding: utf-8 -*-
"""PenZoneA1Strategy 全市场 ETF 日线回测（ATR 止损，对齐 etf_all daily 系列）

与 _bt_pen_zone_a1_stop_etf_all_20210101_20230101_daily 一致：
- PenZoneA1Strategy + PenZoneA1V1（双向 T+0，非 ETF T+1 版）
- 标的：A股场内基金 全部分组
"""
...
FREQ = "日线"
N_JOBS = 4
ROOT_TAG = f"_bt_pen_zone_a1_stop_etf_all_{SDT}_{EDT}_daily"


class PenZoneA1StrategyDaily(PenZoneA1Strategy):
    base_freq = FREQ
...
def main():
    ...
    dummy = DummyBacktest(
        strategy=PenZoneA1StrategyDaily,
    ...
    print(f"回测: PenZoneA1StrategyDaily | {SDT} ~ {EDT} | freq={FREQ}")
