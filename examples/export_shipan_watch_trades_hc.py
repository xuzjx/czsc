# -*- coding: utf-8 -*-
"""导出 shipan 观察模式仓位变化表，并与投研 SQhc9001 回测逐笔对比。

用法：
    python examples/export_shipan_watch_trades_hc.py
"""
...
WATCH_CSV = os.path.join(OUT_DIR, "shipan_watch_pos_changes_hc_2021_2023.csv")
COMPARE_CSV = os.path.join(OUT_DIR, "shipan_vs_research_hc_trades_compare_2021_2023.csv")
BT_ROOT_TAG = f"_bt_pen_zone_a1_hc_one_{SDT}_{EDT}_15m"


class PenZoneA1StrategyHc(PenZoneA1Strategy):
    base_freq = FREQ
...
def research_pairs_to_trades(dfp: pd.DataFrame) -> pd.DataFrame:
    ...
    return out.sort_values("open_dt").reset_index(drop=True)
...
def run_shipan_watch() -> pd.DataFrame:
    ...
    # 前复权与 TqSdk 主力连续默认一致；投研侧用后复权，信号时点应对齐 bar 收盘时刻
